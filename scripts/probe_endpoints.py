"""
Live endpoint probe for AAAA-Nexus / atomadic.tech

Usage:
    python scripts/probe_endpoints.py [--filter GROUP]

Reads AAAA_NEXUS_API_KEY from the .env file (or environment).
Calls every client method with minimal test data.
Prints ✅ / ❌ per endpoint and a final summary table.

Groups tested:
  platform · inference · oracle · ratchet · rng · vrf · identity · escrow
  reputation · sla · discovery · swarm · security · compliance · defi
  aegis · control · consensus · quota · certify · saga · memory · text
  data · governance · billing · dev · bitnet · vanguard · mev · forge
"""
...
# ── bootstrap path so we can import ass_ade without installing ───────────────
...
from ass_ade.nexus.client import NexusClient  # noqa: E402

BASE_URL = os.environ.get(
    "AAAA_NEXUS_BASE_URL",
    os.environ.get("AAAA_NEXUS_URL", "https://atomadic.tech"),
)
API_KEY = os.environ.get("AAAA_NEXUS_API_KEY", "")

if not API_KEY:
    print("ERROR: AAAA_NEXUS_API_KEY not set")
    sys.exit(1)
...
results: list[tuple[str, str, str, float]] = []  # (group, name, status, ms)
...
def run_all(nx: NexusClient, only: str | None = None) -> None:
    ...
    if not only or "inference" in only:
        probe("inference", "inference",         lambda: nx.inference("Say hello in one word.", model="falcon3-10B-1.58"))
    ...
    if not only or "bitnet" in only:
        ...
        probe("bitnet", "bitnet_inference", lambda: nx.bitnet_inference("Reply with one word: hello", "falcon3-10B-1.58"))
        probe("bitnet", "bitnet_benchmark", lambda: nx.bitnet_benchmark("falcon3-10B-1.58", 20))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Live endpoint probe for AAAA-Nexus")
    parser.add_argument("--filter", "-f", default=None,
                        help="Only probe endpoints in groups matching this string (e.g. 'bitnet')")
    parser.add_argument("--base-url", default=BASE_URL)
    args = parser.parse_args()

    print(f"\nAAAA-Nexus endpoint probe  ->  {args.base_url}{RESET}")
    print(f"{DIM}API key: set (redacted){RESET}\n")

    with NexusClient(base_url=args.base_url, api_key=API_KEY) as nx:
        run_all(nx, only=args.filter)

    print_summary()
    sys.exit(1 if any(s == "FAIL" for _, _, s, _ in results) else 0)

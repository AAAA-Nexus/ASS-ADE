...
def test_vector_memory_store_and_query(tmp_path: Path) -> None:
    ...
    assert result.min_score_applied == 0.0
    assert result.below_threshold_omitted == 0


def test_query_vector_memory_min_score_filters_weaker_matches(tmp_path: Path) -> None:
    store_vector_memory(
        text="python asyncio gather cancellation context managers",
        namespace="ns",
        working_dir=tmp_path,
    )
    store_vector_memory(
        text="banana bread oven temperature unrelated",
        namespace="ns",
        working_dir=tmp_path,
    )
    loose = query_vector_memory(
        query="asyncio gather python",
        namespace="ns",
        top_k=5,
        working_dir=tmp_path,
        min_score=0.0,
    )
    tight = query_vector_memory(
        query="asyncio gather python",
        namespace="ns",
        top_k=5,
        working_dir=tmp_path,
    ...
    assert loose.min_score_applied == 0.0
    assert tight.min_score_applied == 0.25
    assert tight.below_threshold_omitted >= loose.below_threshold_omitted
    assert len(tight.matches) <= len(loose.matches)


def test_query_vector_memory_respects_ass_ade_memory_min_score_env(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    store_vector_memory(
        text="identical trust phrase for threshold",
        namespace="env",
        working_dir=tmp_path,
    )
    monkeypatch.setenv("ASS_ADE_MEMORY_MIN_SCORE", "0.85")
    hi = query_vector_memory(
        query="identical trust phrase for threshold",
        namespace="env",
        working_dir=tmp_path,
    )
    assert hi.min_score_applied == 0.85
    assert len(hi.matches) == 1
    assert hi.matches[0].score >= 0.85

    monkeypatch.setenv("ASS_ADE_MEMORY_MIN_SCORE", "0.999")
    empty = query_vector_memory(
        query="different query unrelated zebra",
        namespace="env",
        working_dir=tmp_path,
    )
    assert empty.matches == []
    assert empty.below_threshold_omitted >= 1

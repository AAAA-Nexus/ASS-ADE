"""Phase 1 integration tests — SAM pipeline, LSE in live loop, BAS on depth breach."""
...
from ass_ade.agent.loop import AgentLoop, DELEGATION_DEPTH_CAP
...
# ─────────────────────────────────────────────────────────────────────────────
# BAS alert fires on delegation-depth breach
# ─────────────────────────────────────────────────────────────────────────────


class TestBASOnDelegationDepthBreach:
    def test_depth_breach_returns_blocked_message(self):
        provider = _make_provider_returning_text("ok")
        registry = ToolRegistry()
        orchestrator = EngineOrchestrator({})
        loop = AgentLoop(provider=provider, registry=registry, orchestrator=orchestrator)

        # Simulate being inside a recursion by setting depth past the cap.
        loop._delegation_depth = DELEGATION_DEPTH_CAP + 1
        result = loop.step("[REFINE round 1/3] retry")
        assert "delegation" in result.lower() or "[BLOCKED]" in result

    def test_depth_breach_emits_bas_alert(self):
        provider = _make_provider_returning_text("ok")
        registry = ToolRegistry()
        orchestrator = EngineOrchestrator({})
        loop = AgentLoop(provider=provider, registry=registry, orchestrator=orchestrator)

        # Set to exactly the cap so the next increment exceeds it.
        loop._delegation_depth = DELEGATION_DEPTH_CAP
        loop.step("[REFINE round 1/3] retry")

        # BAS should have a delegation_depth_breach alert.
        alerts = orchestrator.bas._alerts
        kinds = [a.kind for a in alerts]
        assert "delegation_depth_breach" in kinds, f"Expected delegation_depth_breach in {kinds}"

    def test_depth_breach_severity_is_high(self):
        from ass_ade.agent.bas import _SEVERITY
        assert _SEVERITY.get("delegation_depth_breach") == "high"

    def test_normal_flow_does_not_trigger_depth_alert(self):
        provider = _make_provider_returning_text("ok")
        registry = ToolRegistry()
        orchestrator = EngineOrchestrator({})
        loop = AgentLoop(provider=provider, registry=registry, orchestrator=orchestrator)
        ...
        alerts = orchestrator.bas._alerts
        kinds = [a.kind for a in alerts]
        assert "delegation_depth_breach" not in kinds

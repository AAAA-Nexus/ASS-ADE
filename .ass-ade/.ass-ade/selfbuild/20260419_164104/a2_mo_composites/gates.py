"""Quality gates — AAAA-Nexus security and quality checks for agent operations.

Active only in hybrid/premium mode. Each method returns None on failure
(fail-open) so the agent never blocks due to a transient gate error.

Gate pipeline (applied per agent step):
  1. scan_prompt()     — input injection detection
  2. shield_tool()     — tool call security
  3. check_hallucination() — output truthfulness (ε-KL divergence)
  4. certify()         — output quality rubric
  5. trim_context()    — memory optimization via Nexus

In local mode, all gates return None (pass-through). The agent loop
treats None as "gate not available, proceed" — this is the fail-open
design that makes ASS-ADE work with any model, enhanced by Nexus.
"""

...
def _extract_nexus_content(data: Any) -> str:
    ...
    ``{"choices": [{"message": {"content": ...}}]}`` shape returned by
    ``atomadic.tech/v1/inference``. Surrounding markdown code fences are
    stripped because the upstream model frequently wraps code in them.
    """
    ...
    if not raw:
        ...
        if isinstance(choices, list) and choices:
            ...
            if isinstance(msg, dict) and isinstance(msg.get("content"), str):
                ...
                raw = first["text"]
    return _strip_markdown_fences(raw) if raw else ""

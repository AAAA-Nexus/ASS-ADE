...
def materialize_plan(
    ...
    *,
    ...
    # ── Step 1b: Mirror files into src/{package_name}/ layout ────────────────
    _TIER_CLASSIFICATION: dict[str, str] = {
    ...
    tier_map_files: dict[str, dict[str, str]] = {}
    tier_overlay_files: dict[str, list[str]] = {}
    ...
    for src_path_str, tier in sorted(file_to_tier.items()):
        ...
        tier_src_dir.mkdir(parents=True, exist_ok=True)

        dest_name = src_path.name
        dest_file = tier_src_dir / dest_name
        counter = 1
        while dest_file.exists():
            dest_file = tier_src_dir / f"{src_path.stem}_{counter}{src_path.suffix}"
            counter += 1
        ...
        rel_path = f"src/{package_name}/{tier}/{dest_file.name}"
        tier_map_files[rel_path] = {
            "tier": tier,
            "classification": _TIER_CLASSIFICATION.get(tier, "component"),
        }
        tier_overlay_files.setdefault(tier, []).append(rel_path)

    # Write TIER_OVERLAY.json in each tier directory pointing back to src/
    for tier, src_paths in tier_overlay_files.items():
        tier_dir = target_root / tier
        tier_dir.mkdir(parents=True, exist_ok=True)
        overlay_path = tier_dir / "TIER_OVERLAY.json"
        _assert_under_root(overlay_path, out_dir)
        overlay_path.write_text(
            json.dumps(
                {"src_files": src_paths, "src_package": f"src/{package_name}", "tier": tier},
                indent=2,
                sort_keys=True,
            ) + "\n",
            encoding="utf-8",
    ...
    return {
        ...
        "tier_map": tier_map_files,
        "src_layout_root": src_pkg_root.as_posix() if src_pkg_root.exists() else None,
    }

"""LSE — LLM Selection Engine.

Routes each agent step to the optimal (provider, model) pair based on:
  - SAM TRS composite score
  - EpistemicRouter task complexity
  - Remaining token budget

Model selection is delegated to the free-provider catalog (`agent/providers.py`)
so users with no paid API keys can run ASS-ADE entirely on free providers
(Groq, Gemini, OpenRouter, Cerebras, Ollama, etc.) configured in settings.

Fail-open: if selection fails for any reason, the configured default model
is returned unchanged. If no catalog match is found, LSE emits a decision
pointing to the legacy Claude model ids so tests and hard-coded configs
keep working.
"""
...
# Complexity bucket → minimum tier (post-LSE, this is what SAM/routing produce)
_COMPLEXITY_FLOOR: dict[str, str] = {
    "trivial": TIER_FAST,
    "simple": TIER_FAST,
    "medium": TIER_BALANCED,
    "complex": TIER_BALANCED,
    "critical": TIER_DEEP,
}

# Tier ordering (index = capability rank)
_TIERS = [TIER_FAST, TIER_BALANCED, TIER_DEEP]
_TIER_INDEX: dict[str, int] = {m: i for i, m in enumerate(_TIERS)}

# Legacy fallback — used only when no provider catalog entry matches.
# Keeps behavior sensible when no free providers are configured AND the
# user hasn't set an explicit model override.
_LEGACY_TIER_TO_MODEL: dict[str, str] = {
    TIER_FAST: "claude-haiku-4-5-20251001",
    TIER_BALANCED: "claude-sonnet-4-6",
    TIER_DEEP: "claude-opus-4-7",
}
...
def _max_tier(a: str, b: str) -> str:
    ...
    return a if _TIER_INDEX.get(a, 1) >= _TIER_INDEX.get(b, 1) else b

...
def collect_monadic_tiers(working_dir: str | Path = ".") -> list[CapabilityEntry]:
    ...
    def _find_tier_dir(name: str) -> bool:
        """Check root, then up to 3 levels deep for the tier directory."""
        if (root / name).is_dir():
            return True
...
_LIVE_CAP_MARKER_START = "<!-- LIVE_CAPABILITIES_START -->"
_LIVE_CAP_MARKER_END = "<!-- LIVE_CAPABILITIES_END -->"


def write_live_capabilities_md(
    working_dir: str | Path = ".",
    *,
    sibling_dirs: list[Path] | None = None,
    output_path: str | Path | None = None,
) -> Path:
    """Generate agents/LIVE_CAPABILITIES.md from the live capability inventory.

    Called at chat startup and via `ass-ade agents-refresh`.  Writes a markdown
    file that agent prompts can reference for up-to-date capability context.
    """
    root = _repo_root_from_working_dir(working_dir)
    if output_path is None:
        out = root / "agents" / "LIVE_CAPABILITIES.md"
    else:
        out = Path(output_path)
    out.parent.mkdir(parents=True, exist_ok=True)

    snap = build_capability_snapshot(root)
    now = snap.generated_at_utc
    ...
    if sibling_dirs is None:
        ...
        try:
            siblings = sorted(
                ...
                and not any(x in d.name for x in ("backup", "rebuilt", "evolved",
                                                    "venv", "pytest", "QUARANTINE"))
            )[:20]
            sibling_dirs = siblings
        except OSError:
            sibling_dirs = []

    for sib in (sibling_dirs or []):
        try:
            n_py = sum(1 for _ in sib.rglob("*.py")
                       if ".venv" not in str(_) and "__pycache__" not in str(_))
            pyproject = sib / "pyproject.toml"
            if not pyproject.exists():
                pyproject = sib / "atomadic-engine" / "pyproject.toml"
            version = ""
            if pyproject.exists():
            ...
            sibling_lines.append(f"- `{sib.name}`{tag} — {n_py} Python files")
        except OSError:
            sibling_lines.append(f"- `{sib.name}` — (scan error)")

    # --- environment ---
    import sys as _sys
    py_ver = f"{_sys.version_info.major}.{_sys.version_info.minor}.{_sys.version_info.micro}"

    tier_lines = [f"- `{e.name}` — {e.description}" for e in snap.monadic_tiers]
    cli_groups = sorted({e.name.split()[0] for e in snap.cli_commands})
    surface_lines = [f"- `{e.name}` — {e.description}" for e in snap.surface_locations]

    sections = [
        f"# Atomadic Live Capability Map",
        f"",
        f"> Auto-generated: {now}  ",
        f"> Repo root: `{snap.repo_root}`  ",
        f"> Python: {py_ver}",
        f"",
        f"## Capability Counts",
        f"",
        f"| Surface | Count |",
        f"|---------|-------|",
        f"| CLI command groups | {len(cli_groups)} |",
        f"| Local agent tools | {snap.counts['local_tools']} |",
        f"| MCP stdio tools | {snap.counts['mcp_tools']} |",
        f"| Monadic tier dirs | {snap.counts['monadic_tiers']} |",
        f"| Surface locations | {snap.counts['surface_locations']} |",
        f"",
        f"## CLI Command Groups",
        f"",
        f"`{'`, `'.join(cli_groups)}`",
        f"",
        f"## Monadic Tier Structure",
        f"",
        *(tier_lines if tier_lines else ["- none detected"]),
        f"",
        f"## Artifact Surface Locations",
        f"",
        *(surface_lines if surface_lines else ["- none detected"]),
    ]

    if sibling_lines:
        sections += [
            f"",
            f"## Sibling Repos (in `{root.parent.name}/`)",
            f"",
            *sibling_lines,
        ]
    ...
    sections += [
        ...
        f"## Routing Rules",
        f"",
        f"- Prefer exact CLI command names from the list above.",
        f"- For code edits, place files in the correct monadic tier directory.",
        f"- Surface artifact paths are relative to the repo root `{root.name}/`.",
        f"- This file is regenerated at `ass-ade chat` startup and via `ass-ade agents-refresh`.",
        f"- If this file is stale, run `ass-ade agents-refresh` to update it.",
        f"",
        f"---",
        f"*Generated by `write_live_capabilities_md()` in `ass_ade.agent.capabilities`*",
    ]
...
def inject_capabilities_into_agents(
    working_dir: str | Path = ".",
    *,
    glob_pattern: str = "agents/*.prompt.md",
    extra_dirs: list[Path] | None = None,
    dry_run: bool = False,
) -> list[Path]:
    """Inject the live capability section into agent prompt files.

    Replaces the block between ``<!-- LIVE_CAPABILITIES_START -->`` and
    ``<!-- LIVE_CAPABILITIES_END -->`` markers with fresh capability content.

    For files inside the repo (matching ``glob_pattern``), a file-reference
    pointer is written so the content stays in sync without bloating prompts.
    For files in ``extra_dirs`` (e.g. global Claude Code agents at
    ``~/.claude/agents/``), the capability content is **inlined** because
    those files live outside the repo tree.

    Returns the list of files updated.
    """
    root = _repo_root_from_working_dir(working_dir)
    cap_md = root / "agents" / "LIVE_CAPABILITIES.md"
    if not cap_md.exists():
        write_live_capabilities_md(working_dir)

    ref_inject_block = (
        f"{_LIVE_CAP_MARKER_START}\n"
        f"<!-- This section is auto-injected. Edit the markers, not the content. -->\n"
        f"@../agents/LIVE_CAPABILITIES.md\n"
        f"{_LIVE_CAP_MARKER_END}"
    )

    # For external dirs we inline the content directly.
    try:
        inline_content = cap_md.read_text(encoding="utf-8")
    except OSError:
        inline_content = ""
    inline_inject_block = (
        f"{_LIVE_CAP_MARKER_START}\n"
        f"<!-- Auto-injected from {cap_md} — run `ass-ade ade refresh` to update. -->\n"
        f"{inline_content}\n"
        f"{_LIVE_CAP_MARKER_END}"
    )

    def _apply(prompt_file: Path, inject_block: str) -> bool:
        try:
            text = prompt_file.read_text(encoding="utf-8")
        except OSError:
            return False
        if _LIVE_CAP_MARKER_START not in text or _LIVE_CAP_MARKER_END not in text:
            return False
        start = text.index(_LIVE_CAP_MARKER_START)
        end = text.index(_LIVE_CAP_MARKER_END) + len(_LIVE_CAP_MARKER_END)
        new_text = text[:start] + inject_block + text[end:]
        if new_text == text:
            return False
        if not dry_run:
    ...
    updated: list[Path] = []
    for prompt_file in sorted(root.glob(glob_pattern)):
        if _apply(prompt_file, ref_inject_block):
            updated.append(prompt_file)

    for extra_dir in (extra_dirs or []):
        extra_dir = Path(extra_dir)
        for prompt_file in sorted(extra_dir.glob("*.md")):
            if _apply(prompt_file, inline_inject_block):
                updated.append(prompt_file)

...
# Directories that are always excluded (tooling, caches, build artefacts).
_BASE_EXCLUDED_DIRS = {
...
# Tier directory names.  Excluded ONLY when ASS_ADE_SKIP_TIER_DIRS=1 so that
# previously-rebuilt monadic trees (V20, ASS-CLAW) can be ingested as inputs.
# Set ASS_ADE_SKIP_TIER_DIRS=1 to restore the original behaviour (e.g. when
# running rebuild on the ass-ade repo itself to avoid re-ingesting own output).
_TIER_DIRS = {
...
EXCLUDED_DIRS: frozenset[str] = frozenset(
    _BASE_EXCLUDED_DIRS | (_TIER_DIRS if os.environ.get("ASS_ADE_SKIP_TIER_DIRS", "").lower() in ("1", "true", "yes") else set())
)

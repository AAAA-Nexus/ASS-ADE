"""File history and undo stack for agentic edits.

Every file mutation (write_file, edit_file) is recorded as a snapshot
before the change is applied. The agent or user can then undo edits
to restore previous states.

Storage uses a lightweight on-disk approach:
    .ass-ade/history/<sha256_of_path>/<sequence>.snapshot

Each snapshot is a JSON envelope:
    { "path": "rel/path", "sequence": N, "timestamp": "...", "content": "..." }

The undo stack is bounded: only the last ``max_depth`` snapshots per file
are retained to prevent unbounded disk growth.
"""

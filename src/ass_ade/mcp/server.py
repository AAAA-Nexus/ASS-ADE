"""ASS-ADE MCP Server — expose tools and workflows over the MCP protocol.

Implements a JSON-RPC 2.0 stdio transport compatible with the Model Context
Protocol (MCP) 2025-11-25 specification. This lets ASS-ADE run as a tool
server for VS Code Copilot, Claude Desktop, or any MCP-compatible client.

Usage:
    ass-ade mcp serve                # stdio transport (default)
    ass-ade mcp serve --port 3100    # HTTP SSE transport (future)

Exposed tools:
    - All 8 built-in IDE tools (read_file, write_file, edit_file, run_command,
      list_directory, search_files, grep_search, undo_edit)
    - Hero workflows: trust_gate, certify_output, safe_execute (when Nexus is configured)
    - Planning: epiphany_breakthrough_cycle (Phase 0 + structured epiphany→breakthrough JSON)
    - Agent: ask_agent (sends a task through the full agent loop)
    - A2A: a2a_validate, a2a_negotiate (agent card validation and negotiation)

MCP 2025-11-25 features implemented:
    - Tool annotations (readOnlyHint, destructiveHint, idempotentHint, openWorldHint)
    - _meta.progressToken in tools/call — emits notifications/progress for long calls
    - notifications/cancelled — cancels in-flight requests by ID
    - tools/list cursor-based pagination
    - logging capability (notifications/message)
"""
...
# ── Annotations for built-in IDE tools ───────────────────────────────────────
# MCP 2025-11-25: annotations are hints to clients about tool behaviour.
# readOnlyHint   — tool does not modify state
# destructiveHint — tool may destroy data (writes, deletes)
# idempotentHint  — identical calls produce the same result
# openWorldHint   — tool may interact with the outside world (network, OS)

_BUILTIN_ANNOTATIONS: dict[str, dict[str, bool]] = {
    "read_file": {
        "readOnlyHint": True,
...
# ── Workflow / A2A tool schemas for MCP ──────────────────────────────────────

_WORKFLOW_TOOLS: list[dict[str, Any]] = [
    {
        "name": "phase0_recon",
        "description": (
            "Never Code Blind gate: inspect the local codebase and require "
            "current technical-document source targets before capability "
            "planning or coding."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "task_description": {
                    "type": "string",
                    "description": "Technical task to recon before execution.",
                },
                "provided_sources": {
                    "type": "array",
                    "description": "Already-researched official source URLs attached to this task.",
                    "items": {"type": "string"},
                },
                "max_relevant_files": {
                    "type": "integer",
                    "description": "Maximum relevant files to return from codebase recon.",
                },
            },
    ...
    },
    {
        ...
        "description": (
            "Epiphany → Breakthrough envelope: optional Phase 0 recon, planner track, "
            "and a versioned JSON document (epiphanies, hypotheses, experiments, "
            "promotion checks) for COEVO-style execution-grounded evolution."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "task_description": {
                    "type": "string",
                    "description": "Goal or task statement driving recon and planning.",
                },
                "observations": {
                    "type": "array",
                    "description": "Short grounded observations (failing tests, paths, quotes).",
                    "items": {"type": "string"},
                },
                "provided_sources": {
                    "type": "array",
                    "description": "Official source URLs already attached to this task.",
                    "items": {"type": "string"},
                },
                "run_phase0": {
                    "type": "boolean",
                    "description": "If true (default), run phase0_recon before composing the document.",
                },
                "max_relevant_files": {
                    "type": "integer",
                    "description": "Cap on recon file paths embedded in the document.",
                },
            },
            "required": ["task_description"],
        },
    ...
    },
    {
        "name": "context_pack",
        "description": (
            "Build a compact task context packet from Phase 0 recon, selected "
            "repo files, and source URLs."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "task_description": {
                    "type": "string",
                    "description": "Task the packet should support.",
                },
                "file_paths": {
                    "type": "array",
                    "description": (
                        "Optional repo-relative files to include. Defaults to "
                        "recon-selected files."
                    ),
                    "items": {"type": "string"},
                },
                "source_urls": {
                    "type": "array",
                    "description": "Official source URLs already researched for this task.",
                    "items": {"type": "string"},
                },
                "max_files": {
                    "type": "integer",
                ...
                "max_bytes_per_file": {
                    "type": "integer",
                    "description": "Maximum bytes of excerpt per file.",
                },
            },
            "required": ["task_description"],
        },
    ...
    },
    {
        ...
        "description": "Store trusted text into the local vector memory namespace.",
        "inputSchema": {
            "type": "object",
            "properties": {
                ...
                "namespace": {
                    "type": "string",
                    "description": "Memory namespace, usually repo or task ID.",
                },
                "metadata": {
                    "type": "object",
    ...
    },
    {
        ...
        "description": (
            "Query the local vector memory namespace with optional cosine-similarity "
            "floor (min_score); weak matches are omitted from results."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                ...
                "min_score": {
                    "type": "number",
                    "description": (
                        "Minimum cosine similarity (dot on normalized hashing vectors). "
                        "Omit to use ASS_ADE_MEMORY_MIN_SCORE env or 0.0."
                    ),
                },
            },
            "required": ["query"],
    ...
    },
    {
        "name": "map_terrain",
        "description": (
            "MAP = TERRAIN capability gate: verify required agents, hooks, "
            "skills, tools, harnesses, prompts, and instructions exist before "
            "execution; halt and produce an invention packet when they do not."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "task_description": {
                    "type": "string",
                    "description": "Task to execute after capability validation.",
                },
                "required_capabilities": {
                    "type": "object",
                    "description": (
                        "Capability requirements grouped by agents, hooks, "
                        "skills, tools, harnesses, prompts, and instructions."
                    ),
                },
                "agent_id": {
                    "type": "string",
                    "description": "Agent requesting the workflow.",
                },
                "max_development_budget_usdc": {
                    "type": "number",
                    "description": "Maximum budget for inventing missing assets.",
                },
                "auto_invent_if_missing": {
                    "type": "boolean",
                    "description": (
                        "If true, generate repo-native assets and a certified "
                        "tiered rebuild packet for missing capabilities."
                    ),
                },
                "invention_constraints": {
                    "type": "object",
                    "description": "Verification constraints for invented assets.",
                },
            },
    ...
    },
    {
        "name": "trust_gate",
        "description": (
            "Multi-step agent trust gating via AAAA-Nexus: identity, sybil, "
            "trust, reputation, and gate decision. Returns ALLOW, WARN, or DENY."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "agent_id": {
                    "type": "string",
                    "description": "Unique agent identifier to evaluate.",
                },
            },
            "required": ["agent_id"],
        },
    ...
    },
    {
        "name": "certify_output",
        "description": (
            "Certify text output via AAAA-Nexus: hallucination oracle, ethics, "
            "compliance, and certification with lineage tracking."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "text": {"type": "string", "description": "Text content to certify."},
            },
            "required": ["text"],
        },
        "annotations": {
    ...
    },
    {
        "name": "safe_execute",
        "description": (
            "Execute a tool call through AAAA-Nexus safety pipeline: security "
            "shield, prompt scan, AEGIS proxy, and certification."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                ...
                "tool_input": {
                    "type": "string",
                    "description": "JSON-encoded input for the tool.",
                },
            },
    ...
    },
    {
        "name": "ask_agent",
        "description": (
            "Send a task to the ASS-ADE agent loop. Uses the configured model "
    ...
    },
    {
        "name": "a2a_validate",
        "description": (
            "Fetch and validate a remote agent card from a URL per the A2A "
            "specification."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "url": {
                    "type": "string",
                    "description": (
                        "Base URL of the agent; /.well-known/agent.json will "
                        "be appended."
                    ),
                },
            },
    ...
    },
    {
        "name": "a2a_negotiate",
        "description": (
            "Compare the local ASS-ADE agent card with a remote agent card for "
        ...
        "inputSchema": {
            ...
            "properties": {
                "remote_url": {
                    "type": "string",
                    "description": "Base URL of the remote agent to negotiate with.",
                },
            },
            "required": ["remote_url"],
        },

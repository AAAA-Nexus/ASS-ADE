...
_PROVIDER_REGISTRY: dict[str, tuple[str, str | None, str | None, str]] = {
    "aaaa-nexus":   ("https://atomadic.tech/v1/inference",                      "AAAA_NEXUS_API_KEY",  "llama3-8B-1.58-100B",                    "x-api-key"),
...
class Atomadic:
    ...
    # Forge approval gate (epiphany → show plan → user approves → execute forge)
    _pending_forge_plan: Any = field(default=None, init=False, repr=False)
    _pending_forge_output: str = field(default="", init=False, repr=False)
    ...
    def process(self, user_input: str) -> str:
        ...
        # ── Forge approval gate (epiphany plan → user confirms → execute) ──────
        if self._pending_forge_plan is not None:
            approve_words = {"yes", "all", "go", "apply", "forge", "do it",
                             "proceed", "run it", "yep", "yeah", "sure", "ok",
                             "all of them", "everything", "looks good"}
            reject_words = {"no", "cancel", "stop", "skip all", "none",
                            "forget it", "nope", "don't", "hold off", "later"}
            if words & approve_words or any(p in lower_text for p in
                                            ("all of them", "go ahead", "do it", "looks good")):
                plan = self._pending_forge_plan
                output = self._pending_forge_output
                self._pending_forge_plan = None
                self._pending_forge_output = ""
                forge_output = self._execute_forge_from_plan(plan, output, filter_text=text)
                turn = Turn(user=text, tone=tone, intent="forge", path=output,
                            command=[], output=forge_output, response=forge_output)
                self.history.append(turn)
                self.memory.update_from_turn(turn)
                self.memory.append_history(turn)
                self.memory.save()
                return forge_output
            if words & reject_words or any(p in lower_text for p in ("not now", "hold off", "forget it")):
                output = self._pending_forge_output
                self._pending_forge_plan = None
                self._pending_forge_output = ""
                return ("Got it — rebuild is complete without forge. "
                        f"Output is at `{output}`. Run `ass-ade rebuild --forge` anytime to apply improvements.")

        # ── Epiphany planning triggers (show plan before forge) ──────────────
        _epiphany_triggers = (
            "plan a rebuild", "plan rebuild", "preview improvements",
            "show me what you'd", "show me what to improve", "what would you improve",
            "show improvements", "plan the improvements", "epiphany mode",
            "plan enhancements", "analyze before", "show the plan", "preview forge",
            "what can you fix", "what improvements",
        )
        if any(t in lower_text for t in _epiphany_triggers):
            path = self._extract_path(text) or str(self.working_dir)
            ts = datetime.now().strftime("%Y%m%d-%H%M%S")
            output_path = str(Path(path).parent / f"{Path(path).name}-epiphany-{ts}")
            print(f"\n🔮 Epiphany mode: rebuild + plan (no forge yet)", flush=True)
            raw_output = self._execute_epiphany_planning(path, output_path)
            # Return the formatted plan directly — do NOT pass through _postlude
            # which would replace it with a generic "rebuilt" summary.
            turn = Turn(user=text, tone=tone, intent="epiphany", path=path,
    ...
    def _dispatch(
        ...
        if intent == "rebuild":
            ...
            forge_requested = True  # always forge in chat — use epiphany triggers to plan first
            if not output_path:
                ts = datetime.now().strftime("%Y%m%d-%H%M%S")
            ...
            print(prelude, flush=True)
            raw_output = self._execute_rebuild_pipeline(path, output_path, forge=forge_requested)
            return raw_output, []
    ...
    def _execute_rebuild_pipeline(self, source: str, output: str, forge: bool = False) -> str:
        """Built-in rebuild pipeline: backup → copy → recon → lint → docs → certify → hot-patch.

        When forge=True, delegates to `ass-ade rebuild --forge` for full Epiphany+Forge phases.
        """
        ...
        phase_label = "6 phases (with Forge)" if forge else "5 phases"
        print(f"\n🧠 Intent: rebuild")
        print(f"🔧 Dispatching: rebuild pipeline ({phase_label})")
        print(f"   Source : {source_path}")
        print(f"   Output : {output_path}")
        if forge:
            print(f"   Forge  : enabled (Epiphany + ForgeLoop)")
        print(flush=True)

        # When forge is requested, delegate entirely to `ass-ade rebuild --forge`
        # which runs Ingest → Classify → Materialize → Epiphany → Forge → Audit → Certify.
        if forge:
            cmd = base + ["rebuild", str(source_path), "--output", str(output_path), "--yes", "--forge"]
            print(f"🔥 Epiphany → Forge pipeline starting...", flush=True)
            print(flush=True)
            rc = self._run_streaming(cmd)
            self._hot_patch()
            if rc == 0:
                return f"[ok] Forge rebuild complete\nOutput : {output_path}"
            return f"[error] Forge rebuild exited with code {rc}\nOutput : {output_path}"
    ...
    def _execute_epiphany_planning(self, source: str, output: str) -> str:
        """Run basic rebuild then Epiphany analysis; store plan for user approval."""
        source_path = Path(source).resolve()
        output_path = Path(output).resolve()
        base = [sys.executable, "-m", "ass_ade"]

        if source_path == output_path:
            return "[error] Source and output are the same path."

        print(f"\n🔮 Epiphany planning: rebuild first, then scan for improvements")
        print(f"   Source : {source_path}")
        print(f"   Output : {output_path}", flush=True)
        print(flush=True)

        cmd = base + ["rebuild", str(source_path), "--output", str(output_path), "--yes"]
        rc = self._run_streaming(cmd)
        if rc != 0:
            return f"[error] Rebuild failed (exit {rc}) — cannot run Epiphany analysis."

        print(f"\n🔮 Running Epiphany scan on rebuilt output...", flush=True)
        try:
            from ass_ade.engine.rebuild.forge import generate_plan
            plan = generate_plan(output_path)
            tasks = getattr(plan, "experiments", [])
            self._pending_forge_plan = plan
            self._pending_forge_output = str(output_path)

            if not tasks:
                self._pending_forge_plan = None
                return (f"[ok] Rebuild complete — no improvements detected. "
                        f"Output: {output_path}")

            lines = [f"\n✨ Epiphany found **{len(tasks)} improvement(s)** in `{output_path.name}`:\n"]
            for i, task in enumerate(tasks, 1):
                issue_label = task.issue.replace("_", " ").title()
                lines.append(f"  {i}. `[{task.file}]` **{task.node}** — {issue_label}")
            lines.append(
                f"\n**Ready to forge?** Say `yes` to apply all, list numbers to skip any, "
                f"or say `no` to keep the current rebuild as-is."
            )
            return "\n".join(lines)
        except Exception as exc:
            self._pending_forge_plan = None
            return (f"[ok] Rebuild complete at {output_path}\n"
                    f"[warn] Epiphany analysis unavailable: {exc}")

    def _execute_forge_from_plan(self, plan: Any, output: str, filter_text: str = "") -> str:
        """Execute Epiphany plan (or filtered subset) on an already-rebuilt output."""
        output_path = Path(output).resolve()
        print(f"\n🔥 Forge executing approved tasks on {output_path.name}...", flush=True)
        try:
            from ass_ade.engine.rebuild.forge import execute_plan, _FORGE_MODEL, _MAX_WORKERS
            model = _FORGE_MODEL
            workers = _MAX_WORKERS

            # If user said "skip X" or "just Y", filter tasks
            tasks = list(getattr(plan, "experiments", []))
            if tasks and filter_text:
                skip_match = re.search(r'skip\s+(\d+(?:[,\s]+\d+)*)', filter_text, re.IGNORECASE)
                only_match = re.search(r'(?:just|only)\s+(\d+(?:[,\s]+\d+)*)', filter_text, re.IGNORECASE)
                if skip_match:
                    skip_nums = {int(n.strip()) for n in re.findall(r'\d+', skip_match.group(1))}
                    tasks = [t for i, t in enumerate(tasks, 1) if i not in skip_nums]
                elif only_match:
                    only_nums = {int(n.strip()) for n in re.findall(r'\d+', only_match.group(1))}
                    tasks = [t for i, t in enumerate(tasks, 1) if i in only_nums]

            from dataclasses import replace as _dc_replace
            filtered_plan = _dc_replace(plan, experiments=tasks)
            result = execute_plan(filtered_plan, model=model, max_workers=workers)
            applied = getattr(result, "applied", 0)
            total = getattr(result, "plan_tasks", len(tasks))
            return (f"[ok] Forge complete: {applied}/{total} fixes applied\n"
                    f"Output : {output_path}")
        except Exception as exc:
            return f"[error] Forge execution failed: {exc}"
    ...
    def _extract_path(self, text: str) -> str | None:
        ...
        if m:
            ...
            if p.exists():
                return str(p)
        # Unquoted tokens that look like paths
...
def _build_startup_greeting(
    ...
    if has_tier and cert_info:
        last_rebuild = cert_info.get("date", "unknown")
        raw_conf = cert_info.get("conformance", "")
        try:
            conf_num = float(str(raw_conf).rstrip("%"))
            conf_str = f"{conf_num:.1f}% conformant."
        except ValueError:
            conf_str = f"{raw_conf} conformant." if raw_conf else "Conformance not recorded."

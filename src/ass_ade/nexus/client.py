"""NexusClient — typed Python client for all AAAA-Nexus API endpoints.

Covers all 119+ endpoints across 30 product families as documented at
https://atomadic.tech/api-status and https://atomadic.tech/openapi.json

Auth:
    Pass ``api_key`` from a credit-pack or omit for free-tier / x402 calls.
    The key is sent as both ``Authorization: Bearer`` and ``X-API-Key``.

Usage::

    with NexusClient(base_url="https://atomadic.tech", api_key=key) as nx:
        result = nx.hallucination_oracle(text="…")
"""
...
from ass_ade.nexus.errors import raise_for_status
from ass_ade.nexus.validation import safe_path_segment, sanitize_header_value
from ass_ade.nexus.x402 import PaymentChallenge, PaymentProof
...
# Shorthand used in f-string URL paths throughout this module.
# Validates the segment and raises ValueError if it contains path-traversal
# characters or other disallowed chars (OWASP A01).
_pseg = safe_path_segment
...
class NexusClient:
    ...
    def trust_phase_oracle(self, agent_id: str, **kwargs: Any) -> TrustPhaseResult:
        """/v1/oracle/v-ai - geometric trust phase oracle. $0.020/request.

        Returns an opaque phase scalar; callers must not infer thresholds from
        it. See AN-TH-TRUST-FLOOR for the public admission invariant.
        """
    ...
    def ratchet_register(self, agent_id: int, **kwargs: Any) -> RatchetSession:
        """/v1/ratchet/register — new session with epoch counter. agent_id must satisfy the structural-parity invariant `AN-TH-STRUCT-PARITY`. $0.008/call"""
    ...
    def delegate_verify(
        ...
        **kwargs: Any,
        ...
        """/v1/delegate/verify — full UCAN chain validation under the `AN-TH-DEPTH-LIMIT` invariant. $0.080/call"""
        if chain is None:
    ...
    def zero_trust_auth(self, agent_id: int, endpoint: str, capability: str, trust: float = 0.9984, **kwargs: Any) -> ZeroTrustResult:
        """/v1/auth/zero-trust — zero-trust auth primitive. agent_id must satisfy the structural-parity invariant `AN-TH-STRUCT-PARITY`. $0.020/call"""
    ...
    def agent_register(self, agent_id: int, name: str, capabilities: list[str], endpoint: str, **kwargs: Any) -> AgentRegistration:
        """POST /v1/agents/register — agent_id must satisfy the structural-parity invariant `AN-TH-STRUCT-PARITY`. Free"""
    ...
    def trust_gate(self, agent_id: str, action: str = "default", **kwargs: Any) -> dict:
        """Trust gate decision: allowed iff ``trust_score >= AN-TH-TRUST-FLOOR``.

        The numeric floor is resolved server-side from the upstream oracle.
        ...
        try:
            ...
            try:
                ...
                # Fallback floor: resolved from env so the private-IP numeric is
                # never baked into public source. Default is a conservative
                # non-signaling value; real runs receive the floor from the
                # oracle via /v1/trust/gate.
                floor = float(_os.environ.get("ASS_ADE_TRUST_FALLBACK_FLOOR", "0.99"))
                return {
                    "allowed": score_val >= floor,
                    "score": score_val,

...
- Monadic package `ass_ade_v11` (distribution `ass-ade`): phased rebuild pipeline 0–7 (recon through package emit).
- Library entry `rebuild_project_v11`; unified runner `pipeline_book.run_book_until` with numeric stop-after.
- Typer CLI surface under `ass-ade book` with `--stop-after` (`recon` ... `package`); module entry `python -m ass_ade_v11.a4_sy_orchestration`.

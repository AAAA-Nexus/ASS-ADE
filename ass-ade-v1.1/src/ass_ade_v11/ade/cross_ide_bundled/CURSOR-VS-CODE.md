...
| **VS Code + OpenAI Codex** (extension) | Same repo; use **model picker** and optional **MCP** | Auth via the Codex / ChatGPT flow. Tooling: terminal + `ass-ade` like Copilot. |

**Single repo, many IDEs:** Clone once. Materialize with `ass-ade ade materialize` (from an `ass-ade` install) so every machine gets **`.ade/`** with the same operator layout.

**Invariants (MAP = TERRAIN):**

- **Release gates** = `ass-ade book` / `ass-ade assimilate` / CI — not a specific editor.

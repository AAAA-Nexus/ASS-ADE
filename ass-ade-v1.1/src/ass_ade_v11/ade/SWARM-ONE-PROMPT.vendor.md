...
-->
# One-paste â€” ASS-ADE dev swarm (Cursor Â· VS Code Copilot Â· Codex)

**Repo:** `ATOMADIC_WORKSPACE` = this monorepo root. **You already ran** (or will run) host prep in a terminal (see **Host prep** below), then **paste the block in the box** into **Agent/Chat** in **Cursor**, **VS Code Copilot (Agent)**, or **OpenAI Codex** â€” same text for all; they share one git tree.
...
`**AAAA_NEXUS_API_KEY`:** keep it in `**.env`** at repo root; `.vscode/settings.json` loads that file into **integrated terminals**. For **MCP**, ensure Cursorâ€™s process sees the same variable (system env or launch from a prepared shell) â€” see `agents/NEXUS_SWARM_MCP.md` Â§9. **LoRA:** `python scripts/collect_ass_ade_dev_corpus.py` â†’ `training_data/`; then `pip install -e ".[lora]"` and `python scripts/train_lora_local.py` (see Â§9).
...
## â¬‡ï¸ Paste everything inside the next box into your AI â¬‡ï¸

```text
You are the lead ASS-ADE / Atomadic development agent for this repository. MAP = TERRAIN: no stubs, no fake completions, no undocumented engine drops; gap-file or refuse per `agents/_PROTOCOL.md` Â§4.
...
- **Source-of-truth order:** `swarm_task_state.json` + `tasks.json`, then `plan.md` Current position, then `evolution.log`, then ship plan; pulse/signals last â€” full text under **Swarm alignment contract** below this box.
- **Spine (code):** T12 â€” root `pyproject.toml`, monadic package `ass_ade_v11` under `ass-ade-v1.1/src/ass_ade_v11/`. Install story: `pip install -e ".[dev]"` from repo root.
...
- â€œDoneâ€ without a verify command that matches the ship plan

**Begin with A)â€“C) in your first reply, then D).**
```
...
- **Re-paste** the same boxed block for the **next** slice, or say: â€œContinue the same plan from step N.â€
...
| ----------------------- | ---------------------------------------------------------------------------------- |
| Persistent orchestrator | `python scripts/run_swarm_services.py run` or `â€¦ once`                             |

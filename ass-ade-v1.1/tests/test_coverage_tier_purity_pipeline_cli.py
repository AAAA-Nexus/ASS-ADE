...
from ass_ade_v11.a1_at_functions.tier_purity import check_tier_purity, enforce_tier_purity, tier_prefix_from_id
from ass_ade_v11.a3_og_features.phase0_recon import run_phase0_recon
...
from ass_ade_v11.a4_sy_orchestration.run_phases_0_2 import run_book_phases_0_3
...
def test_cli_rebuild_audit_nonconformant_exits_one(
    ...
    monkeypatch.setattr(
        "ass_ade_v11.a4_sy_orchestration.cli.run_book_until",
        fake_book,
    )
    runner = CliRunner()
    r = runner.invoke(
...
def test_resolve_v1_reference_root_is_dir_oserror(
    ...
    from ass_ade_v11.a1_at_functions import v1_reference_index as v1
...
def test_main_py_module_invocation(repo_root: Path) -> None:
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "ass_ade_v11.a4_sy_orchestration", "--help"],
        cwd=str(repo_root),
        capture_output=True,
        text=True,
        check=False,
...
def test_cli_py_executed_as_script(repo_root: Path) -> None:
    cli_path = repo_root / "src" / "ass_ade_v11" / "a4_sy_orchestration" / "cli.py"
...
def test_orchestration_dunder_main_runpy(repo_root: Path) -> None:
    ...
    main_py = repo_root / "src" / "ass_ade_v11" / "a4_sy_orchestration" / "__main__.py"
    old_argv = sys.argv[:]
    try:

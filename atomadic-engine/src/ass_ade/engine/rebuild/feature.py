...
def _parse_components(text: str) -> list[dict[str, Any]] | None:
    stripped = _extract_json_array(text)
    try:
        data = json.loads(stripped)
    except json.JSONDecodeError as exc:
        _record_nexus_error(
            f"feature.propose: JSON parse failed: {exc}; head={stripped[:160]!r}"
        )
        return None
    if not isinstance(data, list):
        _record_nexus_error(
            f"feature.propose: expected JSON array, got {type(data).__name__}"
        )
        return None
    ...
    for item in data:
        ...
        if tier not in _VALID_TIERS:
            skipped.append(f"invalid tier {tier!r} for {name!r}")
            continue
        out.append({
            "name": _slug(name),
            "tier": tier,
    ...
    if not out:
        _record_nexus_error(
            f"feature.propose: 0 valid components; skipped={skipped[:5]}"
        )
        return None
    return out

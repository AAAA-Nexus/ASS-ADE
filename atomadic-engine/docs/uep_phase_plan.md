...
**Pillar**: D_MAX = 23 (Semantic bound), G23 intent/impl gate, TRS scoring live
**Timeframe**: 1–2 weeks
...
| D_MAX=23 depth limiter | NO | **INVENT**: add to AgentLoop |
...
**1.2 — D_MAX=23 Semantic Depth Limiter** (`agent/loop.py`)
...
- D_MAX breaches surface as BAS alerts
...
class LoRAFlywheel:
    ...
    """Every RG_LOOP=47 steps: batch all captures → nexus.lora_contribute"""
...
- Batches every 47 steps (RG_LOOP) → single `nexus.lora_contribute()` call
...
- After each successful `lora_contribute()`: advance session ratchet
  - `nexus.ratchet_advance(session_id=..., epoch=contribution_count // RG_LOOP)`
...
| File | Phase | Changes |
|------|-------|---------|
| `src/ass_ade/agent/loop.py` | 1,4,5 | D_MAX gate, REFINE verdict, conviction pre-gate |
...
| G_18 | 324 | Session ratchet modulus (Phase 5) |
